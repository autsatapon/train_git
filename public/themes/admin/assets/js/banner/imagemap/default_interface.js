...
function gui_input_change(e) {
	...
	else if (obj.name == 'img_pid')    {myimgmap.areas[id].apid   = obj.value;}
...
function gui_addArea(id) {
	...
	var temp = '<input type="text"  name="img['+id+'][id]" class="img_id" value="' + id + '" readonly="1"/>';
	//temp+= '<input type="checkbox" name="img_active" class="img_active" id="img_active_'+id+'" value="'+id+'">';
	//could be checkbox in the future
	temp+= '<input type="radio" name="img['+id+'][img_active]" class="img_active" id="img_active_'+id+'" value="'+id+'">';
	temp+= '<select name="img['+id+'][img_shape]" class="img_shape">';
	...
	temp+= '</select>';
	temp+= 'Coords: <input type="text" name="img['+id+'][img_coords]" class="img_coords" value="">';
	temp+= 'Product ID: <input type="text" name="img['+id+'][img_pid]" class="img_pid" value="">';
	temp+= 'Href: <input type="text" name="img['+id+'][img_href]" class="img_href" value="">';
	temp+= 'Alt: <input type="text" name="img['+id+'][img_alt]" class="img_alt" value="">';
	temp+= 'Target: <select name="img['+id+'][img_target]" class="img_target">';
...
function gui_areaChanged(area) {
	...
	if (props[id]) {
		...
		if (area.apid) {props[id].getElementsByTagName('input')[3].value  = area.apid;}
		if (area.ahref)   {props[id].getElementsByTagName('input')[4].value  = area.ahref;}
		if (area.aalt)    {props[id].getElementsByTagName('input')[5].value  = area.aalt;}
