<div class="mws-panel grid_8">
	...
	<?php if ( !empty($products) && (is_object($products) && (!$products->isEmpty())) ) { ?>
        ...
        <table class="mws-datatable-fn mws-table">
            <thead>
                <tr>
                    <th style="width:150px;"><?php echo Brand::getLabel('name') ?></th>
                    <th><?php echo Product::getLabel('title') ?></th>
                    <th><?php echo Product::getLabel('pkey'),'/',ProductVariant::getLabel('inventory_id') ?></th>
                    <th>Stock</th>
                    <th>Stock Type</th>
            ...
            <tbody>
				...
				<?php foreach ($products as $product) { ?>
					<tr style="border-top:3px solid #4D4D4D;border-bottom:1px solid #4D4D4D;margin-top:10px;">
						 <td class="table-center" style="background-color:#D7D7D7;width:100px"><?php echo $product->brand->name ?></td>
						 <td><strong><?php echo $product->title ?></strong></td>
						 <td class="table-center"><?php echo $product->pkey ?></td>
                   <td></td>
                   <td></td>
						 <td class="table-center">
						 	<a href="<?php echo URL::to('products/edit/'.$product->id); ?>" ><input type="button" class="btn btn-warning" value="Edit"></a>
						...
						<?php foreach ($product->variants as $row => $variant) { ?>
								 ...
								 <tr>
                                    ...
                                    <?php endif; ?>
									 ...
									 <td class="table-right"><?php echo (isset($remainings[$variant->inventory_id]['remaining']))?number_format($remainings[$variant->inventory_id]['remaining']):'-'; ?></td>
									 <td><?php echo $variant->stock_type_code; ?></td>
									 <td></td>
								</tr>
        ...
        <?php }else{ ?>
		        ...
		        <table class="mws-datatable-fn mws-table">
		            <thead>
		                <tr>
							<th class="span1" style="width:150px;"><?php echo Brand::getLabel('name') ?></th>
							<th class="span2"><?php echo Product::getLabel('title') ?></th>
							<th class="span2" rowspan="2"><?php echo Product::getLabel('pkey'),'/',ProductVariant::getLabel('inventory_id') ?></th>
							<th class="span1" style="width:150px;">Action</th>
		                </tr>
		            </thead>
		            <tbody>
